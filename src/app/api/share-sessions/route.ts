...
export async function POST(request: NextRequest) {
  ...
  if (!finalImageUrl && imageDataUrl) {
    ...
    const signed = await supabase.storage.from(bucket).createSignedUrl(objectPath, clampedExpires * 60 + 120);
    if (signed.error || !signed.data?.signedUrl) {
      return NextResponse.json(
        { error: `Create signed URL failed: ${signed.error?.message || 'unknown'}` },
        { status: 500 },
      );
    }
